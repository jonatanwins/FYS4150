...
def single_particle(z0, q, m, V0_d2, filename):  # tz-plot
    ...
    plt.legend()
    plt.grid()
    plt.savefig("plots/one_tz_" + str(filename) + ".pdf")


def two_particles(double_xy, double_xv, double_zv, double_xyz, filename):
    # ------------------------------- Read file -----------------------------
    ...
    if double_xy:
        ...
        ax.grid()
        plt.tight_layout()
        plt.savefig("plots/two_xy_" + str(filename) + ".pdf")
    ...
    if double_xv:
        ...
        ax.grid()
        plt.tight_layout()
        plt.savefig("plots/two_xv_" + str(filename) + ".pdf")
    ...
    if double_zv:
        ...
        ax.grid()
        plt.tight_layout()
        plt.savefig("plots/two_zv_" + str(filename) + ".pdf")
    ...
    if double_xyz:
        ...
        ax.grid()
        plt.tight_layout()
        plt.savefig("plots/two_xyz_" + str(filename) + ".pdf")
...
if __name__ == "__main__":
    ...
    if single:
        ...
        for path in folder_path.iterdir():
            filename = path.name
            single_particle(z0, q, m, V0_d2, filename)

    # -------------------------------------   2 Particles ----------------------------------

    # two_particles(double_xy, double_xv, double_zv, double_xyz, filename)
